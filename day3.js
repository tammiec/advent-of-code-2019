...
const getPath = array => {
  ...
  for (let entry of array) {
    if (entry[0] === 'R') {
      for (let i = 0; i < parseInt(entry.slice(1)); i++) {
        x++;
          pathArray.push(`${x}, ${y}`);
      }
    } else if (entry[0] === 'L') {
...
const getIntersections = (array1, array2) => {
  const path1 = array1;
  const path2 = new Set(array2);
  const intersectionPoints = path1.filter(coord => path2.has(coord));

  return intersectionPoints;
}


const getShortestDistance = array => {
  const nested = array.map(coord => coord.split(', '));

  const distances = nested.map(intersection => Math.abs(0 - intersection[0]) + Math.abs(0 - intersection[1]));

  return distances.sort((a, b) => a - b)[0];
}

const w1 = getPath(wire1);
const w2 = getPath(wire2);

const intersections = getIntersections(w1, w2);
console.log('intersections:', intersections);

console.log('shortest distance', getShortestDistance(intersections));

const getSteps = (array1, array2, intersection) => {
  let w1Steps = array1.indexOf(intersection) + 1;
  let w2Steps = array2.indexOf(intersection) + 1;
  return w1Steps + w2Steps;
}

const shortestSteps = (array1, array2, intersectionArray) => {
  const stepsArray = [];
  for (let intersection of intersectionArray) {
    stepsArray.push(getSteps(array1, array2, intersection));
  }
  
  console.log(stepsArray);
  return stepsArray.sort((a, b) => a - b)[0];
}

console.log('shortest steps', shortestSteps(w1, w2, intersections));